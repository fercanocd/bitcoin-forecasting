...
Model-results figures (07-14) live in results_plots.py and are built from the
saved predictions after the models have run.
...
from src.visualization.style import STYLE, PALETTE, DPI, FIGSIZE, FIG_DIR, _save
...
# ---------------------------------------------------------------------------
# Methodology schematic (two-round protocol) — one consolidated diagram
# ---------------------------------------------------------------------------

def plot_method_overview():
    """Consolidated methodology schematic for the Materials and Methods section.

    A single timeline shows the whole two-round protocol: Round 1 selects
    hyperparameters with four expanding cross-validation folds inside the
    2018-2023 development window (train grows, validation is one calendar year);
    Round 2 freezes them and evaluates by expanding walk-forward on the sealed
    2024-2026 test block. Replaces the three separate method_* placeholders."""
    print("\n[00] Methodology overview (two-round protocol)...")

    c_train, c_val, c_test = "#4C72B0", "#DD8452", "#55A868"
    start, dev_end, test_end = 2018.0, 2024.0, 2026.42   # 2026-06-01 ~ 2026.42
    bar_h = 0.6
    fold_val_years = [2020, 2021, 2022, 2023]

    fig, ax = plt.subplots(figsize=(11, 4.4))
    yticks, ylabels = [], []

    # Round-2 row on top: train on all of dev + expanding walk-forward on test.
    y_test = len(fold_val_years) + 1
    ax.broken_barh([(start, dev_end - start)], (y_test, bar_h), facecolors=c_train)
    ax.broken_barh([(dev_end, test_end - dev_end)], (y_test, bar_h),
                   facecolors=c_test, hatch="////", edgecolor="white")
    yticks.append(y_test + bar_h / 2); ylabels.append("Round 2\ntest")

    # Round-1 folds below (Fold 1 on top, Fold 4 at the bottom).
    for i, vy in enumerate(fold_val_years):
        y = len(fold_val_years) - i
        ax.broken_barh([(start, vy - start)], (y, bar_h), facecolors=c_train)
        ax.broken_barh([(vy, 1.0)], (y, bar_h), facecolors=c_val)
        yticks.append(y + bar_h / 2); ylabels.append(f"Fold {i + 1}")

    ax.axvline(dev_end, color="0.35", ls="--", lw=1.1)

    ax.set_yticks(yticks); ax.set_yticklabels(ylabels, fontsize=9)
    ax.set_ylim(0.4, y_test + bar_h + 1.0)
    ax.set_xlim(start - 0.15, test_end + 0.15)
    ax.set_xticks(list(range(2018, 2027)))
    ax.set_xticklabels([str(y) for y in range(2018, 2027)], fontsize=9)
    ax.set_xlabel("Year")

    top = y_test + bar_h + 0.12
    ax.text((start + dev_end) / 2, top, "Development window (2018–2023)",
            ha="center", va="bottom", fontsize=9, fontweight="bold")
    ax.text((dev_end + test_end) / 2, top, "Test (2024–2026)",
            ha="center", va="bottom", fontsize=9, fontweight="bold")

    handles = [
        mpatches.Patch(color=c_train, label="Train (expanding)"),
        mpatches.Patch(color=c_val,   label="Validation (1 year)"),
        mpatches.Patch(facecolor=c_test, hatch="////", edgecolor="white",
                       label="Test walk-forward"),
    ]
    ax.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, -0.16),
              ncol=3, fontsize=8, frameon=False)

    ax.set_title("Two-round evaluation protocol: Round-1 CV folds and the sealed test window",
                 fontsize=11)
    for spine in ("top", "right", "left"):
        ax.spines[spine].set_visible(False)
    ax.tick_params(left=False)
    fig.tight_layout()
    _save(fig, "00_method_overview.png")
...
def plot_all_eda():
    ...
    with plt.style.context(STYLE):
        ...
        plot_acf_pacf()
    print("\nAll EDA plots saved.")
    print("Result figures (07-16) are generated by results_plots.py after the models run.")
